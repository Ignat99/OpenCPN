...
Label::Label( void )
{
    ...
    pLabelPointList = new LabelPointList;
    ...
    m_PlannedSpeed = LABEL_DEFAULT_SPEED;
    if(g_PlanSpeed != LABEL_DEFAULT_SPEED)
...
Label::~Label( void )
{
    pLabelPointList->DeleteContents( false );            // do not delete Marks
    pLabelPointList->Clear();
    delete pLabelPointList;
...
//
void Label::CloneLabel( Label *psourcelabel, int start_nPoint, int end_nPoint, const wxString & suffix)
{
    m_bIsTrack = psourcelabel->m_bIsTrack;

    m_LabelNameString = psourcelabel->m_LabelNameString + suffix;
    m_LabelStartString = psourcelabel->m_LabelStartString;
    m_LabelEndString = psourcelabel->m_LabelEndString;
    ...
    for( i = start_nPoint; i <= end_nPoint; i++ ) {
        if( !psourcelabel->m_bIsInLayer ) AddPoint( psourcelabel->GetPoint( i ), false );
            ...
            LabelPoint *ptargetpoint = new LabelPoint( psourcepoint->GetIconName(), psourcepoint->GetNo(), psourcepoint->GetName(), psourcepoint->GetCName(), psourcepoint->GetPack(), psourcepoint->GetPcs(), psourcepoint->GetLast(), psourcepoint->GetCode(), psourcepoint->GetImage(), psourcepoint->GetOrder(), psourcepoint->GetDate(), GPX_EMPTY_STRING, false );

            AddPoint( ptargetpoint, false );

            CloneAddedLabelPoint( m_pLastAddedPoint, psourcepoint );
        }
    }
...
void Label::CloneTrack( Label *psourcelabel, int start_nPoint, int end_nPoint, const wxString & suffix)
{
    if( psourcelabel->m_bIsInLayer ) return;
    ...
    int startTrkSegNo;
    if( b_splitting ) startTrkSegNo = psourcelabel->GetPoint( start_nPoint )->m_GPXTrkSegNo;
    else
    ...
    for( i = start_nPoint; i <= end_nPoint; i++ ) {
        ...
        //    This is a hack, need to undo the action of Label::AddPoint
//        ptargetpoint->m_bIsInLabel = false;
...
void Label::CloneAddedLabelPoint( LabelPoint *ptargetpoint, LabelPoint *psourcepoint )
{
...
void Label::CloneAddedTrackPoint( LabelPoint *ptargetpoint, LabelPoint *psourcepoint )
{
    //    This is a hack, need to undo the action of Label::AddPoint
    ptargetpoint->m_MarkDescription = psourcepoint->m_MarkDescription;
    ptargetpoint->m_bIsVisible = psourcepoint->m_bIsVisible;
...
void Label::AddPoint( LabelPoint *pNewPoint, bool b_rename_in_sequence, bool b_deferBoxCalc, bool b_isLoading )
{
    pLabelPointList->Append( pNewPoint );
...
void Label::AddTentativePoint( const wxString& GUID )
{
    LabelPointGUIDList.Add( GUID );
    return;
}

LabelPoint *Label::GetPoint( int nWhichPoint )
{
    LabelPoint *prp;
    wxLabelPointListNode *node = pLabelPointList->GetFirst();
...
LabelPoint *Label::GetPoint( const wxString &guid )
{
    LabelPoint *prp;
    wxLabelPointListNode *node = pLabelPointList->GetFirst();
...
void Label::DrawPointWhich( ocpnDC& dc, int iPoint, wxPoint *rpn )
{
    if( iPoint <= GetnPoints() )
        GetPoint( iPoint )->Draw( dc, rpn );
}
/*
void Label::DrawSegment( ocpnDC& dc, wxPoint *rp1, wxPoint *rp2, ViewPort &VP, bool bdraw_arrow )
{
    if( m_bRtIsSelected ) dc.SetPen( *g_pLabelMan->GetSelectedLabelPen() );
    else
        if( m_bRtIsActive ) dc.SetPen( *g_pLabelMan->GetActiveLabelPen() );
        else
            dc.SetPen( *g_pLabelMan->GetLabelPen() );

    RenderSegment( dc, rp1->x, rp1->y, rp2->x, rp2->y, VP, bdraw_arrow );
}
*/

void Label::Draw( ocpnDC& dc, ViewPort &VP )
{
    if( m_nPoints == 0 ) return;

    if( m_bVisible && m_bRtIsSelected ) {
//        dc.SetPen( *g_pLabelMan->GetSelectedLabelPen() );
//        dc.SetBrush( *g_pLabelMan->GetSelectedLabelBrush() );
    }
    else if ( m_bVisible )
    {
//        int style = wxSOLID;
//        int width = g_label_line_width;
        wxColour col;
//        if( m_style != STYLE_UNDEFINED ) style = m_style;
//        if( m_width != STYLE_UNDEFINED ) width = m_width;
        if( m_Colour == wxEmptyString ) {
//            col = g_pLabelMan->GetLabelPen()->GetColour();
        } else {
...
void Label::DrawGL( ViewPort &VP, OCPNRegion &region )
{
#ifdef ocpnUSE_GL
    if( m_nPoints < 1 || !m_bVisible ) return;
    
    //  Hiliting first
    //  Being special case to draw something for a 1 point label....
    ocpnDC dc;
    if(m_hiliteWidth){
...
//    if( m_bRtIsActive )
//    {
//        wxPen &pen = *g_pLabelMan->GetActiveLabelPen();
//        col = pen.GetColour();
//        width = pen.GetWidth();
//    } else if( m_bRtIsSelected ) {
//        wxPen &pen = *g_pLabelMan->GetSelectedLabelPen();
//        col = pen.GetColour();
//        width = pen.GetWidth();
//    } else {
//        if(m_bIsTrack)
...
//    }
    ...
    for(wxLabelPointListNode *node = pLabelPointList->GetFirst();
...
//            cc1->GetCanvasPointPix( prp->m_lat, prp->m_lon, &rpt2 );
            if(node != pLabelPointList->GetFirst())
    ...
    /*  Label points  */
    for(wxLabelPointListNode *node = pLabelPointList->GetFirst(); node; node = node->GetNext()) {
        LabelPoint *prp = node->GetData();
        if ( !m_bVisible )
...
void Label::RenderSegment( ocpnDC& dc, int xa, int ya, int xb, int yb, ViewPort &VP,
...
void Label::RenderSegmentArrowsGL( int xa, int ya, int xb, int yb, ViewPort &VP)
{
#ifdef ocpnUSE_GL
    //    Draw a direction arrow        
    wxPoint icon[10];
...
void Label::ClearHighlights( void )
{
    LabelPoint *prp = NULL;
    wxLabelPointListNode *node = pLabelPointList->GetFirst();
        ...
        bool bRenamePoints )
{
    LabelPoint *newpoint = new LabelPoint( wxString( _T ( "diamond" ) ), wxString( _T ( "diamond" ) ), wxString( _T ( "diamond" ) ), 
            wxString( _T ( "diamond" ) ), wxString( _T ( "diamond" ) ), wxString( _T ( "diamond" ) ), wxString( _T ( "diamond" ) ), wxString( _T ( "diamond" ) ), wxString( _T ( "diamond" ) ),
            GetNewMarkSequenced(), GPX_EMPTY_STRING );
    newpoint->m_bDynamicName = true;
    newpoint->SetNameShown( false );

    int nRP = pLabelPointList->IndexOf( pRP );
    pLabelPointList->Insert( nRP, newpoint );

    LabelPointGUIDList.Insert( pRP->m_GUID, nRP );

    m_nPoints++;

    if( bRenamePoints ) RenameLabelPoints();
...
wxString Label::GetNewMarkSequenced( void )
{
    wxString ret;
...
LabelPoint *Label::GetLastPoint()
{
    LabelPoint *data_m1 = NULL;
    wxLabelPointListNode *node = pLabelPointList->GetFirst();
...
int Label::GetIndexOf( LabelPoint *prp )
{
    int ret = pLabelPointList->IndexOf( prp ) + 1;
...
void Label::DeletePoint( LabelPoint *rp, bool bRenamePoints )
{
    //    n.b. must delete Selectables  and update config before deleting the point
    if( rp->m_bIsInLayer ) return;

//    pSelect->DeleteAllSelectableLabelPoints( this );
//    pSelect->DeleteAllSelectableLabelSegments( this );
//    pConfig->DeleteWayPoint( rp );

    pLabelPointList->DeleteObject( rp );

    if( ( rp->m_GUID.Len() ) && ( wxNOT_FOUND != LabelPointGUIDList.Index( rp->m_GUID ) ) ) LabelPointGUIDList.Remove(
            rp->m_GUID );
...
//    if( bRenamePoints ) RenameLabelPoints();
    ...
    if( m_nPoints > 1 ) {
//        pSelect->AddAllSelectableLabelSegments( this );
//        pSelect->AddAllSelectableLabelPoints( this );

//        pConfig->UpdateLabel( this );
...
void Label::RemovePoint( LabelPoint *rp, bool bRenamePoints )
{
//    if( rp->m_bIsActive && this->IsActive() )                  //FS#348
//    g_pLabelMan->DeactivateLabel();

//    pSelect->DeleteAllSelectableLabelPoints( this );
//    pSelect->DeleteAllSelectableLabelSegments( this );

    pLabelPointList->DeleteObject( rp );
    if( wxNOT_FOUND != LabelPointGUIDList.Index( rp->m_GUID ) ) LabelPointGUIDList.Remove(
            rp->m_GUID );
    m_nPoints -= 1;

    // check all other labels to see if this point appears in any other label
//    Label *pcontainer_label = g_pLabelMan->FindLabelContainingWaypoint( rp );

//    if( pcontainer_label == NULL ) {
//        rp->m_bIsInLabel = false;          // Take this point out of this (and only) label
...
void Label::ReloadLabelPointIcons()
{
    wxLabelPointListNode *node = pLabelPointList->GetFirst();
...
void Label::FinalizeForRendering()
{
    m_bNeedsUpdateBBox = true;
}

wxBoundingBox Label::GetBBox( void )
{
...
*/
        ...
        //    For Labels that cross the IDL, we compute and store
...
bool Label::CalculateCrossesIDL( void )
{
    wxLabelPointListNode *node = pLabelPointList->GetFirst();
    if( NULL == node ) return false;

    bool idl_cross = false;
    LabelPoint *data = node->GetData();             // first node
...
void Label::CalculateDCRect( wxDC& dc_label, wxRect *prect, ViewPort &VP )
{
    dc_label.ResetBoundingBox();
    dc_label.DestroyClippingRegion();
    
    wxRect update_rect;

    // Draw the label in skeleton form on the dc
    // That is, draw only the label points, assuming that the segements will
...
//            prp2->m_bBlink = false;
            ocpnDC odc_label( dc_label );
            prp2->Draw( odc_label, NULL );
...
/*
 Update the label segment lengths, storing each segment length in <destination> point.
 Also, compute total route length by summing segment distances.
 */
void Label::UpdateSegmentDistances( double planspeed )
{
    wxPoint rpt, rptn;
    float slat1, slon1, slat2, slon2;

    double label_len = 0.0;
    double label_time = 0.0;
...
//                                prp0->m_seg_etd = etd.ToUTC();
    ...
    m_label_length = label_len;
    m_label_time = label_time * 3600.;
}

void Label::Reverse( bool bRenamePoints )
{
    RebuildGUIDList();                        // ensure the GUID list is intact and good
    ...
    int ncount = LabelPointGUIDList.GetCount();
    for( int i = 0; i < ncount; i++ )
        ArrayTemp.Add( LabelPointGUIDList[ncount - 1 - i] );

    LabelPointGUIDList = ArrayTemp;

    pLabelPointList->DeleteContents( false );
    pLabelPointList->Clear();
    m_nPoints = 0;
    m_label_length = 0.0;
    
    AssembleLabel();                          // Rebuild the label points from the GUID list

    if( bRenamePoints ) RenameLabelPoints();

    // Switch start/end strings. anders, 2010-01-29
    wxString tmp = m_LabelStartString;
    m_LabelStartString = m_LabelEndString;
    m_LabelEndString = tmp;
}

void Label::RebuildGUIDList( void )
{
    LabelPointGUIDList.Clear();               // empty the GUID list
    ...
    while( node ) {
        rp = node->GetData();
        LabelPointGUIDList.Add( rp->m_GUID );

        node = node->GetNext();
    }
}
void Label::SetVisible( bool visible, bool includeWpts )
{
    m_bVisible = visible;
...
void Label::SetListed( bool visible )
{
    m_bListed = visible;
}

void Label::AssembleLabel( void )
{
    //    iterate over the LabelPointGUIDs
    for( unsigned int ip = 0; ip < LabelPointGUIDList.GetCount(); ip++ ) {
        wxString GUID = LabelPointGUIDList[ip];

        //    And on the LabelPoints themselves
/*
        wxLabelPointListNode *prpnode = pWayPointMan->GetWaypointList()->GetFirst();
        while( prpnode ) {
            LabelPoint *prp = prpnode->GetData();

            if( prp->m_GUID == GUID ) {
                AddPoint( prp );
                break;
            }
            prpnode = prpnode->GetNext(); //LabelPoint
        }
*/
    }
}

void Label::RenameLabelPoints( void )
{
    //    iterate on the label points.
