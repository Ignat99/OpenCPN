...
// Copy Constructor
LabelPoint::LabelPoint( LabelPoint* orig )
{
...
//        pWayPointMan->RemoveLabelPoint( this );
...
void LabelPoint::Draw( ocpnDC& dc, wxPoint *rpn )
{
    wxPoint r;
    wxRect hilitebox;
...
//        hi_colour = GetGlobalColor( _T ( "YELO1" ) );
        wxColour hi_colour = GetGlobalColor( _T ( "YELO1" ) );
        transparency = 150;
//    }
...
//    if( m_bBlink && ( gFrame->nBlinkerTick & 1 ) ) bDrawHL = true;
    if( gFrame->nBlinkerTick & 1 ) bDrawHL = true;
...
#ifdef ocpnUSE_GL
void LabelPoint::DrawGL( ViewPort &vp, OCPNRegion &region )
{
    if( !m_bIsVisible )
    return;
...
void LabelPoint::CalculateDCRect( wxDC& dc, wxRect *prect )
{
    dc.ResetBoundingBox();
